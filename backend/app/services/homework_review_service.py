...
ПРАВИЛА ОФОРМЛЕНИЯ И ОЦЕНИВАНИЯ (обязательно):
...
3. ОЦЕНКА (от 1 до 5) выставляется строго по следующей матрице (Базовая оценка 5):
   - ФУНКЦИОНАЛЬНОСТЬ: Если код падает с ошибкой или не решает задачу (минус 2-3 балла). Если падает только в пограничных случаях (минус 1 балл).
   - ЧИТАЕМОСТЬ: За грязный код, плохие имена переменных или отсутствие отступов (минус 1 балл).
   - ОПТИМАЛЬНОСТЬ: Нерациональные алгоритмы (например, двойные циклы вместо словаря) (минус 1 балл).
   - ТЕСТЫ/КВИЗЫ: 1-2 ошибки (минус 1 балл). Более 3 ошибок (оценка не выше 3).
   - Специфические критерии из описания задания (description) всегда в приоритете.
4. Структура ответа (пропускай пункты, которых нет в задании):
   - Краткий итог (1–2 предложения)
   - Разбор кода (пункты)
   - Разбор тестов с вариантами (если были)
   - Разбор письменной части (пункты)
   - Рекомендации
   - В конце: ПРЕДЛАГАЕМАЯ_ОЦЕНКА: N  (целое от 1 до 5)
   - Следующей строкой: ОШИБКИ_JSON: ["точная подстрока из кода/текста", ...]
5. Не выдумывай ошибки; если работа идеальна — похвали и поставь 5. Не используй markdown-заголовки с # и не заменяй span на жирный шрифт.
"""
...
def review_assignment(homework: Homework, assignment: HomeworkAssignment) -> dict:
    ...
    criteria_section = ""
    if getattr(homework, "ai_criteria", None):
        criteria_section = f"\n\n--- СЕКРЕТНЫЕ КРИТЕРИИ ПРЕПОДАВАТЕЛЯ (ИМЕЮТ НАИВЫСШИЙ ПРИОРИТЕТ ПРИ ОЦЕНКЕ) ---\n{homework.ai_criteria}\n"

    user_prompt = f"""ЗАДАНИЕ:
{homework.description[:HOMEWORK_REVIEW_TEXT_LIMIT]}{criteria_section}
