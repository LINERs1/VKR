...
/** Нормализация HTML отзыва (как на бэкенде feedback_html). */
export function normalizeFeedbackHtml(text) {
  let t = String(text || '')
  t = t.replace(
...
/** Безопасный HTML отзыва с подсветкой ошибок для v-html. */
export function renderFeedbackHtml(html) {
  if (!html) return ''
